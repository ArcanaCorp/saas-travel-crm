...
export default function FormEditClient ({ toggle, data, edit }) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            setLoading(true);
            const response = await updateClient(data?.id, form, user?.agency_id);
            edit(response);
    ...
    return (
        ...
        <form className="w bg-surface border rounded-md p-md" style={{"--w": "400px", "--mnw": "400px"}} onSubmit={handleSubmit}>
            ...
            <div className="w-full flex flex-col gap-md">
                ...
                <div className="w-full">
                    <button className="btn btn-block btn-primary" disabled={loading}>{loading ? 'Editando...' : 'Editar cliente'}</button>
                </div>
            </div>
        </form>
